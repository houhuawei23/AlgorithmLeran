    ...
    for (int i = 1; i <= q; i++) {
        ...
        for (int j = 1; j <= p; j++) {
            ...
            if (((!adx || !ady) || (adx==ady)) && (adx || ady)) {
                ...
                int dx = posx[j] - u;
                int dy = posy[j] - v;
                if (dx>0 && dy==0) tmp[j]=0;
                else if(dx>0 && dy>0) tmp[j]=1;
                else if(dx==0 && dy>0) tmp[j]=2;
                else if(dx<0 && dy>0) tmp[j]=3;
                else if(dx<0 && dy==0) tmp[j]=4;
                else if(dx<0 && dy<0) tmp[j]=5;
                else if(dx==0 && dy<0) tmp[j]=6;
                else if(dx>0 && dy <0) tmp[j]=7;
        ...
        if (k > 0) { // 更新所有（8各方位上k距离处的）员工的位置
            ...
            for (int j = 1; j <= p; j++) {
                ...
                if (((!adx || !ady) || (adx==ady)) && (adx || ady) && (max(adx, ady) == k)) {
                    // cout << "ad: " << adx << ", " << ady << endl;

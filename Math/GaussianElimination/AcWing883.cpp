...
int gauss() {
    ...
    for (c = 0, r = 0; c < n; c++) {
        ...
        if (fabs(a[t][c]) < eps)
            continue;
        // 如果当前这一列的最大数都是 0 ，那么所有数都是
        // 0，就没必要去算了，因为它的约束方程，可能在上面几行
        for (int i = c; i < n + 1; i++)
            swap(a[t][i], a[r][i]);
        // 把当前这一行，换到最上面（不是第一行，是第 r 行）去
        for (int i = n; i >= c; i--)
            a[r][i] /= a[r][c];
        // 把当前这一行的第一个数，变成 1， 方程两边同时除以
        // 第一个数，必须要到着算，不然第一个数直接变1，系数就被篡改，后面的数字没法算
        for (int i = r + 1; i < n; i++)  // 把当前列下面的所有数，全部消成 0
            if (fabs(a[i][c]) > eps)  // 如果非0 再操作，已经是 0就没必要操作了
                for (int j = n; j >= c; j--)
                    // 从后往前，当前行的每个数字，都减去对应列 *
                    // 行首非0的数字，这样就能保证第一个数字是 a[i][0] -=
                    // 1*a[i][0];
                    a[i][j] -= a[r][j] * a[i][c];
    ...
    if (r < n) {
        // 说明剩下方程的个数是小于 n
        // 的，说明不是唯一解，判断是无解还是无穷多解
        // 因为已经是阶梯型，所以 r ~ n-1 的值应该都为 0
        for (int i = r; i < n; i++)
            if (fabs(a[i][n]) > eps)
                // a[i][n] 代表 b_i ,即 左边=0，右边=b_i,0
                // != b_i, 所以无解。
                return 2;
